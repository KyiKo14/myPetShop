              ...
              child: Row(children: [
                Container(
                  ...
                  child: ClipRRect(
                    ...
                    child: Image.asset('assets/category_image/logo01.jpeg', height: 45, width: 45, fit: BoxFit.cover),
                  ),
                ),
            ...
            if (_searchQuery.isEmpty) ...[
              ...
              Padding(
                ...
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('Shop By Category', style: TextStyle(fontSize: 18, color: Colors.grey.shade900, fontWeight: FontWeight.bold)),
                  const Text('See All', style: TextStyle(fontSize: 14, color: Colors.deepPurple, fontWeight: FontWeight.w600)),
                ]),
              ),
              StreamBuilder(
                ...
                builder: (context, AsyncSnapshot<QuerySnapshot> snap) {
                  ...
                  return SingleChildScrollView(
                    ...
                    child: Row(children: List.generate(snap.data!.docs.length, (i) => Padding(
                      padding: EdgeInsets.only(left: i == 0 ? 20 : 0, right: 16),
                      child: InkWell(
                        onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) =>
                            CategoryItems(selectedCategory: snap.data!.docs[i]['name'], category: snap.data!.docs[i]['name']))),
                        ...
                        child: Column(children: [
                          Container(
                            ...
                            child: CircleAvatar(radius: 35, backgroundColor: category[i].fbackgroundcolor1,
                                backgroundImage: AssetImage(category[i].image)),
                          ),
                          const SizedBox(height: 8),
                          Text(category[i].name, style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600, color: Colors.grey.shade800)),
                        ]),
                      ),
                    ))),
                  );
              ...
              // ==================== SHOP FOR YOU HEADLINE & SEE ALL BUTTON ====================
              Padding(
                key: _shopForYouKey,
                padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 20),
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                  Text('Shop For You', style: TextStyle(fontSize: 18, color: Colors.grey.shade900, fontWeight: FontWeight.bold)),
              ...
              Padding(
                ...
                child: Text('Results for "$_searchQuery"', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
              ),
            ],
            StreamBuilder<QuerySnapshot>(
              ...
              builder: (context, AsyncSnapshot<QuerySnapshot> snapshot) {
                ...
                if (docs.isEmpty) {
                  return Padding(padding: const EdgeInsets.all(40),
                    child: Center(child: Column(children: [
                      ...
                      const SizedBox(height: 8),
                      Text(_searchQuery.isNotEmpty ? 'No results for "$_searchQuery"' : 'No items available.',
                ...
                final items = docs.map((doc) {
                  final d = doc.data() as Map<String, dynamic>;
                  return AppModel(name: d['name'] ?? '', image: d['image'] ?? '',
                      category: d['category'] ?? 'General', price: d['price'] ?? 0,
                      size: List<String>.from(d['sizes'] ?? []), description: d['description'] ?? '',
                      rating: 4.5, review: 12, ischeck: true, fcolor: [Colors.black, Colors.blue, Colors.white]);
                }).toList();
                ...
                if (_searchQuery.isNotEmpty) {
                  return GridView.builder(
                    ...
                    gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(
                        crossAxisCount: 2, childAspectRatio: 0.68, mainAxisSpacing: 16, crossAxisSpacing: 16),
                    itemCount: items.length,
                    itemBuilder: (context, i) => InkWell(
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ItemsDetailScreen(petShop: items[i]))),
                ...
                return SingleChildScrollView(
                  scrollDirection: Axis.horizontal, physics: const BouncingScrollPhysics(),
                  child: Row(children: List.generate(items.length, (i) => Padding(
                    padding: EdgeInsets.only(left: i == 0 ? 20 : 0, right: 16, bottom: 16),
                    child: InkWell(
                      onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ItemsDetailScreen(petShop: items[i]))),
